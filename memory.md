...
* **Post-generation hash recording:** after generation completes (both streaming and non-streaming branches of `chat_completions_endpoint`), the server pre-computes the assistant's `h[k] = _hash_assistant_turn(prev, content_after_thinking_strip, parsed_tool_calls)` and appends it to the session's chain. Hashing the post-strip content (= what the client receives) maximizes round-trip stability. If client canonicalization differs (whitespace etc.), the next request matches one turn short and re-prefills the assistant — wasteful but correct. Reasonable clients (OWUI, OpenAI SDK) echo byte-for-byte what was streamed; mismatch is rare.
...
Audit + buildout to close the high/medium-severity unit-test gaps for local mods. Net (rolling): **~160 initial + ~50 follow-on (heartbeat watchdog, per-turn hash chain) = ~210 new tests across 5 new files + 4 extended files; full suite 675 passing** (up from baseline 511); 5 pre-existing failures unchanged (audio I/O, two known logging/signature drifts in `test_generate.py`, two upstream-merged Nemotron model tests). Initial backfill committed in `3bca51c`; round-3/4 test extensions committed in `7ae8f28`; heartbeat-watchdog tests in `f16611f`; per-turn-hash chain tests uncommitted as of 2026-05-01.
