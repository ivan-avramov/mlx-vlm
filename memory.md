...
**13. RoPE Desync, Step-Padding, and the SWA Memory Hole (generate.py)**
...
* **The Fix:** Implemented a type-aware "Smart Rewind". For SWA ring buffers, a recursive `_has_rotating_cache()` guard automatically invalidates the prefix cache and forces a full, clean re-prefill to rebuild mathematical integrity (checks nested `CacheList` structures too). For standard dynamic caches, the arrays are unconditionally physically sliced using `_kv_seq_axis()` (which defaults to axis 2 for the standard MLX `[B, H, L, D]` layout when dims are ambiguous) to strip both ghost tokens and step-padding, ensuring RoPE indices remain perfectly aligned.
...
**16. The Ghost Prompt Math Guard (server.py)**
