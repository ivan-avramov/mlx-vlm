...
def scaled_dot_product_attention(
    ...
    if isinstance(cache, TurboQuantKVCache):
        ...
        if result is not None:
            return result
        # Fork: quantized_attention chunks over both Q and K with online softmax
        # + mx.eval between K-tiles, keeping peak memory ~O(chunk) rather than
        # O(context). Upstream dequantizes the whole state and runs dense SDPA.
