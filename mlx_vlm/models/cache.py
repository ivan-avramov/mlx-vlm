...
class BatchKVCache(_BaseCache):
    ...
    def __init__(self, left_padding: List[int], prealloc_tokens: int = 0):
        ...
        # Fork: optional first-fill floor so later appends write in place (no
        # realloc, no growth transient). 0 keeps upstream's step-256 growth.
    ...
    def update_and_fetch(self, keys, values):
        ...
        if self.keys is None or (prev + keys.shape[2]) > self.keys.shape[2]:
            ...
            if self.keys is None and self.prealloc_tokens > 0:
                ...
                cap = max(cap, -(-target // self.step) * self.step)
            k_shape = (B, n_kv_heads, cap, k_head_dim)
            v_shape = (B, n_kv_heads, cap, v_head_dim)
            new_k = mx.zeros(k_shape, keys.dtype)
            new_v = mx.zeros(v_shape, values.dtype)
            if self.keys is not None:
                if prev % self.step != 0:
...
class BatchQuantizedKVCache(_BaseCache):
    ...
    def __init__(
        ...
        self._right_padding = None
        # Fork: see BatchKVCache.prealloc_tokens. 0 keeps upstream's growth.
        self.prealloc_tokens = int(prealloc_tokens or 0)
...
# ---------------------------------------------------------------------------
# Fork-only cache classes.
#
# Everything above this line is vendored from upstream mlx-vlm and should stay
# byte-identical to it apart from the two `prealloc_tokens` hunks (marked
# `# Fork:`) so future `git merge upstream/main` applies cleanly. Add fork work
# below, not above.
# ---------------------------------------------------------------------------
...
class PreallocKVCache(KVCache):
    ...
    def shrink_to_offset(self):
        ...
        capacity = self.keys.shape[2]
        target = max(
            self.step, ((self.offset + self.step - 1) // self.step) * self.step
        )
