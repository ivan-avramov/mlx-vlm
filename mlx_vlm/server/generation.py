...
class ResponseGenerator:
    ...
    def _initialize_model(self):
        ...
        # Fork: warm the thinking-delimiter encode cache while this is still
    ...
    def generate(
        ...
        # Fork: cached-path consumers need the original prompt string
        # (stream_generate accepts a string and re-tokenizes internally against
        # the cache's prefix). Carrying it through the queue lets the daemon
    ...
    def _make_thinking_budget_criteria(
        ...
        # Fork: feed the criteria the model's real delimiters via the registry,
        # preferring a single-token closer so the forced close sequence is
        # exact. The hardcoded <think>/</think> are not real tokens for
        # families like Gemma 4 (opener <|think|>, closer <channel|>); see
        # prompt_utils.THINKING_FORMATS. Mirrors the dispatch.py path.
        fmt = detect_thinking_format(tokenizer.decode(input_ids.flatten().tolist()))
        if fmt is not None:
