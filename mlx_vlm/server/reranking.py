...
from fastapi import (
    HTTPException,  # Fork: upstream's next line is `from mlx_lm.models.base import create_causal_mask`, which this fork drops; the replacement is the `..models.cache` import below. The marker sits here because the removal is a pure-deletion hunk and this is the statement git anchors it to.
)
from pydantic import BaseModel, Field

from ..models.cache import (
    create_causal_mask,  # Fork: upstream imports this name from mlx_lm.models.base; the two implementations are byte-identical, and #1891 dropped mlx-lm from requirements.txt while leaving that module-scope import here -- which broke `import mlx_vlm.server` outright, since app.py imports this module at module scope.
)
