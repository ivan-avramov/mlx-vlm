    ...
    **kwargs,
    ...
    if draft_model is not None:
        ...
        if draft_kind == "suffix":
            ...
            # a plain Python object, so it has no ``.config``. Capture the full
            # prompt token ids here, before chunked prefill can slice
            # ``input_ids`` down to the last token.
            suffix_prompt_token_ids = [int(t) for t in input_ids.reshape(-1).tolist()]
        else:
            # mtp / eagle3 / dflash: upstream's per-draft-kind prefill capture
            # kwargs, merged into ``step_kwargs`` below (not persisted on kwargs).
...
def _make_cache(
    ...
    *kv_quant_scheme* selects the quantization backend:
    ...
    def _make_quant_cache(lp):
        if use_turbo:
            if defer_turbo:
                return cache.BatchKVCache(lp, prealloc_tokens=kv_prealloc_tokens)
...
def _sample_with_positions(
    # Fork: adds the per-row `seeds` passthrough (O30, ab5273f8); upstream's
    # signature ends at `positions`.
...
def _resolve_seeds(
    # Fork: fork-only helper for per-request seeds (O30, ab5273f8).
...
class GenerationBatch:
    ...
    def extend(self, other: "GenerationBatch"):
        ...
        if self.seeds is not None or other.seeds is not None:
            self_seeds = (
                self.seeds if self.seeds is not None else [None] * len(self.uids)
            )
            other_seeds = (
                other.seeds if other.seeds is not None else [None] * len(other.uids)
            )
...
class BatchGenerator:
    ...
    def _build_mixed_prompt_batch(
        ...
        # peers (e.g. a cold quantized row joining an exact float one). The
        # policy carries bits + group_size + scheme (and the key/value split
        # when overridden) so the backend choice matches.
