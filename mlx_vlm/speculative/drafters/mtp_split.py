...
def _config_mtp_file(model_path: Path) -> Optional[Path]:
    # Fork: fork-only. Some converters (e.g. mlx-optiq) relocate the mtp sidecar
    # out of the repo root (so non-recursive *.safetensors globs skip it) and
    # record its real path in config.json instead; upstream globs the root only.
...
class MTPSplitter:
    ...
    def iter_selected(
        ...
        # Fork: prefer a config.json-declared mtp sidecar over the root glob;
        # see _config_mtp_file.
        sidecar = _config_mtp_file(source_path)
        candidates = [sidecar] if sidecar else _safetensor_files(source_path)
