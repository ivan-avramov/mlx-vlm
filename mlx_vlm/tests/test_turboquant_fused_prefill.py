...
def test_reference_matches_bruteforce_numpy():
    ...
    q = mx.array(
        np.random.default_rng(2).standard_normal((B, n_kv * r, 1, D)).astype(np.float32) * 0.1
    )
...
def test_mse_prefill_matches_reference_small_noncausal():
    ...
    q = mx.array(
        np.random.default_rng(4).standard_normal((B, n_kv * r, 64, D)).astype(np.float32) * 0.1
    )
    scale = 1.0 / math.sqrt(D)
    ref = reference_attention(q, cache, ks, vs, scale, causal=False)
    out = cache.mse_prefill(q, cache._unwrap(ks), cache._unwrap(vs), scale=scale, mask=None)
...
def test_mse_prefill_causal_matches_reference():
    ...
    L = 256
    q = mx.array(
        np.random.default_rng(6).standard_normal((B, n_kv * r, L, D)).astype(np.float32) * 0.1
    )
    scale = 1.0 / math.sqrt(D)
    ref = reference_attention(q, cache, ks, vs, scale, causal=True)
    out = cache.mse_prefill(q, cache._unwrap(ks), cache._unwrap(vs), scale=scale, mask="causal")
    assert mad(out, ref) < 3e-2
...
def test_mse_prefill_causal_oddshape():
    ...
    L = 128
    q = mx.array(
        np.random.default_rng(8).standard_normal((B, n_kv * r, L, D)).astype(np.float32) * 0.1
    )
    scale = 1.0 / math.sqrt(D)
    ref = reference_attention(q, cache, ks, vs, scale, causal=True)
...
def test_mse_prefill_matrix(D, n_kv, r, bits, L, T):
    ...
    q = mx.array(
        np.random.default_rng(99).standard_normal((1, n_kv * r, L, D)).astype(np.float32) * 0.1
    )
    scale = 1.0 / math.sqrt(D)
    ref = reference_attention(q, cache, ks, vs, scale, causal=True)
    out = cache.mse_prefill(q, cache._unwrap(ks), cache._unwrap(vs), scale, "causal")
...
def test_prefill_attention_none_when_killswitch_off():
    ...
    assert (
        cache.prefill_attention(q, keys_state=ks, values_state=vs, scale=1.0 / 16, mask="causal")
        is None
    )
