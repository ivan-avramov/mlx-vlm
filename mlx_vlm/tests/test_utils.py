...
def test_load_processor_preserves_additional_eos_tokens_on_reset():
    ...
    with (
        ...
        # Fork: our load_processor reads the config first, so load_config must
        # be patched too (same adaptation as the other tests in this file).
...
def test_load_processor_propagates_auto_processor_errors():
    # Fork: `load_config` must be patched too. Our `load_processor` reads the config
    # first (to set laguna's `fix_mistral_regex`), which upstream's does not, so
    # without this the FileNotFoundError from a nonexistent /tmp/model masks the
    # ValueError under test.
    with (
        patch("mlx_vlm.utils.load_config", return_value={}),
