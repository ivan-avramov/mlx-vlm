...
from mlx_vlm.server import session_manager as srv
...
def _reset_session_state():
    ...
    # behave as they did in the fork. Tests needing a specific cap override
    # this locally; the fixture restores the original value after each test.
...
class TestEnvHelpers:
    ...
    def test_env_choice_returns_default_when_unset(self, monkeypatch):
        ...
        assert srv._env_choice("TEST_CHOICE", "auto", ["auto", "on", "off"]) == "auto"
...
class TestCanonicalMessage:
    ...
    def test_excludes_reasoning_even_when_present(self):
        ...
        assert srv._canonical_message(with_reasoning) == srv._canonical_message(without)
...
class TestComputeTurnHashes:
    ...
    def test_chain_property_h1_depends_on_h0(self):
        ...
        a = srv._compute_turn_hashes([_make_msg("system", "A"), _make_msg("user", "u")])
        b = srv._compute_turn_hashes([_make_msg("system", "B"), _make_msg("user", "u")])
        assert a[0] != b[0]
        assert a[1] != b[1]
    ...
    def test_dict_messages_match_pydantic(self):
        ...
        assert srv._compute_turn_hashes(as_objs) == srv._compute_turn_hashes(as_dicts)
    ...
    def test_hash_includes_content_separator_safety(self):
        ...
        # property holds.
        a = srv._compute_turn_hashes([_make_msg("system", "AB"), _make_msg("user", "")])
        b = srv._compute_turn_hashes([_make_msg("system", "A"), _make_msg("user", "B")])
        assert a != b
...
class TestResolveSession:
    ...
    def _req(self, messages, headers=None, chat_id=None, metadata=None):
        raw, parsed = _make_request(headers=headers, chat_id=chat_id, metadata=metadata)
    ...
    def test_explicit_chat_id_wins_over_hash_match(self):
        ...
        srv._sync_session_hashes("anon:existing", srv._compute_turn_hashes(msgs))
    ...
    def test_anon_match_reuses_existing_session(self):
        ...
        srv._sync_session_hashes("anon:original", srv._compute_turn_hashes(msgs))
...
class TestRewindAndEdit:
    ...
    def test_regenerate_last_assistant_keeps_session(self):
        ...
        srv._session_caches["anon:orig"]["turn_hashes"] = srv._compute_turn_hashes(
            prev_request
        )
    ...
    def test_regenerate_does_not_match_below_min_threshold(self):
        ...
        srv._session_caches["anon:other"]["turn_hashes"] = srv._compute_turn_hashes(
