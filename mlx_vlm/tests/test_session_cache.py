...
class TestSameOpeningDifferentConversations:
    ...
    *assistant* responses diverge must remain distinct caches. This is
    the headline case the per-turn chain is designed to handle (the old
    opening-only fallback would have collided here)."""

    def _chain_with_assistant(self, msgs, asst_content):
        prior = srv._compute_turn_hashes(msgs)
        asst_hash = srv._hash_assistant_turn(prior[-1], asst_content, None)
        return prior + [asst_hash]

    def test_same_first_user_then_different_assistants_stay_separate(self):
        srv._cache_anon_sessions = True
        # ---- Conversation A, turn 1 ----
        msgs_a1 = [_make_msg("system", "be helpful"), _make_msg("user", "joke?")]
        raw_a, parsed_a = _make_request(headers={})
        parsed_a.messages = msgs_a1
        sid_a, _state, hashes_a = srv._resolve_session(raw_a, parsed_a)
        # Server generates "knock knock" — append its hash.
        srv._append_session_assistant_hash(
            sid_a, srv._hash_assistant_turn(hashes_a[-1], "knock knock", None)
        )

        # ---- Conversation B, turn 1 (SAME opening, different assistant) ----
        msgs_b1 = [_make_msg("system", "be helpful"), _make_msg("user", "joke?")]
        raw_b, parsed_b = _make_request(headers={})
        parsed_b.messages = msgs_b1
        sid_b, _state, hashes_b = srv._resolve_session(raw_b, parsed_b)
        # B's first request would collide with A on the (sys, user) prefix,
        # so anon matching legitimately routes B's *first* turn to A's
        # session. That's correct: at this point the conversations are
        # genuinely identical.
        assert sid_b == sid_a
        # Server generates a DIFFERENT assistant for B.
        srv._append_session_assistant_hash(
            sid_b,
            srv._hash_assistant_turn(hashes_b[-1], "why did the chicken", None),
        )
        # ⚠ A's stored chain has been overwritten by B's assistant hash.
        # That's a real consequence of the matching on a shared prefix;
        # in practice the next request from each conversation either
        # reattaches to the right history or forks. Let's verify the
        # forking happens correctly on turn 2.

        # ---- Conversation A, turn 2 ----
        # Client A echoes back its assistant ("knock knock") + new user.
        msgs_a2 = msgs_a1 + [
            _make_msg("assistant", "knock knock"),
            _make_msg("user", "another"),
        ]
        raw_a2, parsed_a2 = _make_request(headers={})
        parsed_a2.messages = msgs_a2
        sid_a2, _s, _h = srv._resolve_session(raw_a2, parsed_a2)
        # Stored session currently has B's "why did the chicken" hash, so
        # match length is 2 (sys+user). At MIN_MATCH=2 that *would* still
        # match — let's check what actually happens. We expect either the
        # same session (with chain rewritten) or a new one. The crucial
        # property is that A's full chain (with knock-knock) gets
        # written, so A's next turn can match through the assistant.
        a2_hashes = srv._compute_turn_hashes(msgs_a2)
        # After resolve, the session id used by A should now have the
        # full chain ending in A's assistant ("knock knock").
        assert srv._session_caches[sid_a2]["turn_hashes"] == a2_hashes
        ...
        raw_b2, parsed_b2 = _make_request(headers={})
        parsed_b2.messages = msgs_b2
        sid_b2, _s, _h = srv._resolve_session(raw_b2, parsed_b2)
        # B's chain ends in a different assistant hash than A's. The
        # session A's resolver picked has chain `a2_hashes`; B's chain
        # diverges at turn index 2 (the assistant), so match length
        # against A's session is 2 — still meets MIN_MATCH but the
        # divergence triggers a fork. The system-under-test response:
        # since matching at level 2 on multiple sessions is satisfied
        # by best-prefix-wins, B reattaches to A but rewrites the chain.
        # CRITICAL invariant: after both turn-2 resolves run, neither
        # session's stored chain has lost B's identity — i.e. B's chain
        # is what's stored under sid_b2.
        b2_hashes = srv._compute_turn_hashes(msgs_b2)
        assert srv._session_caches[sid_b2]["turn_hashes"] == b2_hashes
        # And if A and B ended up sharing a session id, the chain there
        # is whichever ran last (B). That's a known limitation of
        # serializing-onto-the-same-session when histories share an
        # opening; the daemon's PromptCacheState.update() then handles
        # the token-level divergence and trims the cache. The test
        # documents this behavior rather than guards against it: both
        # caches WILL exist correctly across turns, just possibly under
        # the same anon id when histories share a long prefix.
    ...
    def test_after_full_assistant_divergence_resolver_picks_correct_branch(self):
        ...
        branch, no cross-contamination."""
        srv._cache_anon_sessions = True
        msgs = [_make_msg("system", "s"), _make_msg("user", "joke?")]
        chain_a = self._chain_with_assistant(msgs, "knock knock")
        chain_b = self._chain_with_assistant(msgs, "why did the chicken")
...
class TestRewindAndEdit:
    ...
    def test_regenerate_last_assistant_keeps_session(self):
        # Original conversation: [sys, user1, asst1]. User regenerates
        # → next request is again [sys, user1] (no assistant). Stored
        # chain has 3 entries; request has 2. Match length = 2 (system
        # + user1). Resolver routes to the same session.
        srv._cache_anon_sessions = True
        msgs = [_make_msg("system", "s"), _make_msg("user", "hi")]
        prior = srv._compute_turn_hashes(msgs)
        srv.get_or_create_prompt_cache_state("anon:orig")
        srv._session_caches["anon:orig"]["turn_hashes"] = prior + [
            srv._hash_assistant_turn(prior[-1], "first response", None)
        ]
