...
class TestSnapshotPathIntegration:
    ...
    def test_asymmetric_path_with_no_rotating_layers(self):
        ...
        snapshots = _capture_rotating_layers_for_snapshot(tracked, capture_rotating)
...
class TestComputeAnchorBeforeLatestUserOffset:
    ...
    def test_returns_offset_at_latest_gemma4_user_turn(self):
        ...
        offset = _compute_anchor_before_latest_user_offset(prompt, _CharTokenizer())
    ...
    def test_returns_none_when_no_marker_present(self):
        ...
        assert (
            _compute_anchor_before_latest_user_offset(prompt, _CharTokenizer()) is None
        )
    ...
    def test_offset_is_prefix_length_not_marker_position_when_tokenizer_merges(self):
        ...
        offset = _compute_anchor_before_latest_user_offset(prompt, _ChunkedTokenizer())
    ...
    def test_falls_back_to_plain_encode_when_tokenizer_rejects_kwarg(self):
        ...
        prompt = "x<|turn>user\ny"
        offset = _compute_anchor_before_latest_user_offset(prompt, _NoKwargTokenizer())
        assert offset == prompt.rfind("<|turn>user\n")
...
class TestOpenWebUIRagWrappingRegression:
    ...
    def gemma_prompts(self):
        ...
        asst2_and_tools = "<|turn>model\n[tool calls + tool responses content]<turn|>\n"
        asst_marker = "<|turn>model\n"
        turn2_prompt = (
            system + user1 + asst1_echo + user2_plain_marker_and_body + asst_marker
        )
        turn3_prompt = (
            system
            + user1
    ...
    def test_anchor_is_stable_across_turns_when_user_message_unchanged(
        ...
        offset_t2 = _compute_anchor_before_latest_user_offset(turn2_prompt, tokenizer)
        offset_t3 = _compute_anchor_before_latest_user_offset(turn3_prompt, tokenizer)
